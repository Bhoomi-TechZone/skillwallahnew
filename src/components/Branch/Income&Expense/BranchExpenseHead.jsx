...
import { getExpenseHeads, createExpenseHead, updateExpenseHead, toggleExpenseHeadStatus } from '../../../api/expenseApi';
import BranchLayout from '../BranchLayout';

const BranchExpenseHead = () => {
  ...
  const loadExpenseHeads = async () => {
    try {
      ...
      toast.error(error.message || 'Failed to load expense heads');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingHead) {
        await updateExpenseHead(editingHead.id, formData);
        toast.success('Expense head updated successfully');
      } else {
        await createExpenseHead(formData);
      ...
      toast.error(error.message || 'Failed to save expense head');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleStatus = async (head) => {
    try {
      await toggleExpenseHeadStatus(head.id);
      toast.success(`Expense head ${head.status ? 'disabled' : 'enabled'} successfully`);
      loadExpenseHeads();
    } catch (error) {
        ...
        <div className="mb-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center space-x-3">
              <h1 className="text-xl md:text-2xl font-bold text-gray-900">Manage Expense Head</h1>
            </div>
            <button
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="3" className="px-6 py-8 text-center text-gray-500">
                      No expense heads found. Click "Add New Head" to create one.
                    </td>
                  </tr>
                ) : (
                  Array.isArray(expenseHeads) && expenseHeads.map((head, index) => (
        ...
        <div className="md:hidden space-y-4">
          {loading ? (
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 text-center text-gray-500">
              No expense heads found. Tap "Add New Head" to create one.
            </div>
          ) : (
            Array.isArray(expenseHeads) && expenseHeads.map((head, index) => (
...
export default BranchExpenseHead;
