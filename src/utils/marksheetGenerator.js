...
export const generateMarksheet = async (studentData, branchData = null) => {
  try {
    ...
    // Use FIXED template only - ignore any fallbacks
    const templateUrl = `${API_BASE_URL}/${FIXED_MARKSHEET_TEMPLATE_PATH}`;
    
    console.log('🔒 [MARKSHEET] Using FIXED template:', templateUrl);

    // Verify template exists
    const response = await fetch(templateUrl, { method: 'HEAD' });
    if (!response.ok) {
      throw new Error(`Fixed marksheet template not found at: ${templateUrl}. Please ensure ${FIXED_MARKSHEET_TEMPLATE_PATH} exists.`);
    }

    console.log('✅ [MARKSHEET] Fixed template verified:', templateUrl);
